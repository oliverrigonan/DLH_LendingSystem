...
{
    public class RepCollectiblesController : Controller
    {
        // data
        private Data.LendingDataContext db = new Data.LendingDataContext();

        // GET: RepCollectibles
        public ActionResult collectibles(Int32? areaId, String dueDate)
        {
            if (areaId != null && dueDate != null)
            {
                ...
                loanApplicationheader.AddCell(new PdfPCell(new Phrase("Collectibles", fontArial17Bold)) { Border = 0, HorizontalAlignment = 2 });
                loanApplicationheader.AddCell(new PdfPCell(new Phrase("Pardo", fontArial12)) { Border = 0, PaddingTop = 5f });
                loanApplicationheader.AddCell(new PdfPCell(new Phrase("Maturity Date " + dueDate, fontArial12)) { Border = 0, PaddingTop = 5f, HorizontalAlignment = 2, });
                ...
                loanHeaderLabel.AddCell(new PdfPCell(new Phrase(area.FirstOrDefault().Area + " Daily Collections", fontArial12White)) { Border = 0, HorizontalAlignment = 0, PaddingTop = 3f, PaddingBottom = 9f, PaddingLeft = 5f, BackgroundColor = BaseColor.BLACK });
                ...
                float[] loanApplicationDataWidthCells = new float[] { 40f, 20f, 20f, 20f };
                ...
                loanApplicationData.AddCell(new PdfPCell(new Phrase("Balance Amount", fontArial12Bold)) { HorizontalAlignment = 1, PaddingTop = 3f, PaddingBottom = 6f, BackgroundColor = BaseColor.LIGHT_GRAY });
                loanApplicationData.AddCell(new PdfPCell(new Phrase("", fontArial12Bold)) { HorizontalAlignment = 1, PaddingTop = 3f, PaddingBottom = 6f, BackgroundColor = BaseColor.LIGHT_GRAY });
                ...
                PdfPTable loanApplicationTotalData = new PdfPTable(4);
                float[] loanApplicationTotalDataWidthCells = new float[] { 40f, 20f, 20f, 20f };
                ...
                loanApplicationTotalData.AddCell(new PdfPCell(new Phrase("TOTAL COLLECTIBLES", fontArial12Bold)) { Border = 0, HorizontalAlignment = 2, PaddingTop = 10f, PaddingBottom = 6f, PaddingLeft = 5f });
                loanApplicationTotalData.AddCell(new PdfPCell(new Phrase(totalBalanceAmount.ToString("#,##0.00"), fontArial12)) { Border = 0, HorizontalAlignment = 2, PaddingTop = 10f, PaddingBottom = 6f, PaddingRight = 5f });
