...
namespace Lending.Reports
{
    public class RepCollectionDetailController : Controller
    {
        ...
        public ActionResult collectionDetail(Int32? collectionId)
        {
            ...
            {
                var collectionIsLocked = from d in db.trnCollections where d.Id == collectionId select d;
                if (collectionIsLocked.Any())
                {
                    if (collectionIsLocked.FirstOrDefault().IsLocked)
                    {
                        // PDF settings
                        ...
                        if (collections.Any())
                        {
                            ...
                            float[] collectionLinesDataWidthCells = new float[] { 12f, 10f, 17f, 16f, 15f, 15f, 15f };
                            ...
                            collectionLinesData.AddCell(new PdfPCell(new Phrase("Collected Amount", fontArial12Bold)) { HorizontalAlignment = 1, PaddingTop = 3f, PaddingBottom = 6f, BackgroundColor = BaseColor.LIGHT_GRAY });
                            ...
                            if (collectionLines.Any())
                            {
                                foreach (var collectionLine in collectionLines)
                                {
                                    ...
                                    totalCollectedAmount += collectionLine.Amount;
                            ...
                            float[] collectionLinesTotalDataWidthCells = new float[] { 12f, 10f, 17f, 16f, 15f, 15f, 15f };
                            ...
                            collectionLinesTotalData.AddCell(new PdfPCell(new Phrase("TOTAL", fontArial12Bold)) { Border = 0, HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 6f, PaddingLeft = 5f });
                            collectionLinesTotalData.AddCell(new PdfPCell(new Phrase(totalCollectedAmount.ToString("#,##0.00"), fontArial12)) { Border = 0, HorizontalAlignment = 2, PaddingTop = 3f, PaddingBottom = 6f, PaddingRight = 5f });
